...
| Receiving.ReceiptStagingIsEventIsolated | PASS | Add Selected stages a complete receipt row and aggregate with Excel events isolated, restores prior event state, and reports the exact failing stage. |
| Receiving.ConfirmUsesQuietUiBoundary | PASS | The real Confirm Writes form handler suppresses repeated Excel save UI and restores the prior application UI state. |
