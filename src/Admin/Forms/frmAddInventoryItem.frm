...
Private Sub mCmbUom_Change()
    ...
    If newUom = "" Then
        mCmbUom.Value = IIf(mPreviousUom <> "", mPreviousUom, "EA")
        Exit Sub
    End If
...
Private Sub ApplyModeLayout()
    ...
    mEditMode = (Not mTabs Is Nothing And mTabs.Value = 1)
    mLblEditItem.Visible = mEditMode
    mCmbEditItem.Visible = mEditMode
    mLblTitle.Caption = IIf(mEditMode, "Edit inventory item", "Add inventory item")
    mLblContext.Caption = IIf(mEditMode, "Edit catalog fields for an existing inventory item.", _
                              "Fill the required fields. Internal item code is generated by invSys.")
    mLblQty.Caption = IIf(mEditMode, "Qty", "Starting qty *")
    mTxtQty.Enabled = Not mEditMode
    mBtnOK.Caption = IIf(mEditMode, "Save Item", "Add Item")
    If Not mEditMode Then
        mSelectedEditSku = ""
        mGeneratedRow = IIf(mGeneratedRow > 0, mGeneratedRow, 0)
